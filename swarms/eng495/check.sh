#!/usr/bin/env bash
# ENG-495 check: `cornerman lint` matches the pinned oracle on every conformance case,
# including the twelve red cases for the phase 1a review's gaps. Runs inside the task's
# git worktree of cornerman.
...
LOG="$(mktemp)"
# Gate against clean dependencies, never the worker's copy.
rm -rf deps _build
...
mkdir -p "$OUT"
git add -A -- lib
git diff --cached HEAD > "$OUT/eng495.patch"
[ -s "$OUT/eng495.patch" ] || fail "exported patch is empty"
cp notes.md "$OUT/notes.md"
echo "PASS: every conformance case matches the oracle, twice; patch at $OUT/eng495.patch"
